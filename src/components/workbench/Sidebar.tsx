...
export function Sidebar({ open, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          'fixed lg:static top-0 left-0 z-40 h-[100dvh] lg:h-auto w-72 p-4 flex flex-col gap-4 transition-transform duration-300',
      ...
      >
        ...
        <div className="flex items-center justify-between px-2 py-2">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h1 className="font-display font-bold text-white text-lg leading-none">WebFtp</h1>
        ...
        <GlassPanel variant="subtle" className="p-4">
          ...
          <div className="flex justify-between mt-2 text-[10px] text-slate-500 font-mono">
            <span>{formatBytes(user?.used_bytes || 0)}</span>
            <span>/ {formatBytes(user?.quota_bytes || 0)}</span>
          </div>
        </GlassPanel>
